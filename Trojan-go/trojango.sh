...
ip6tables -I INPUT -m state --state NEW -m tcp -p tcp --dport 2053 -j ACCEPT
ip6tables -I INPUT -m state --state NEW -m udp -p udp --dport 2053 -j ACCEPT
...
figlet -f slant TROJAN-GO | lolcat
sleep 1
rm -f trojango.sh
