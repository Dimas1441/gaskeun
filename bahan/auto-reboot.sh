...
echo -e "${white}Auto-Reboot Berhasil Diaktifkan Setiap 1 Jam.${off}"
elif test $x -eq 2; then
echo "10 */6 * * * root /usr/local/bin/reboot_otomatis" > /etc/cron.d/reboot_otomatis
echo -e "${white}Auto-Reboot Berhasil Diaktifkan Setiap 6 Jam.${off}"
elif test $x -eq 3; then
echo "10 */12 * * * root /usr/local/bin/reboot_otomatis" > /etc/cron.d/reboot_otomatis
echo -e "${white}Auto-Reboot Berhasil Diaktifkan Setiap 12 Jam.${off}"
elif test $x -eq 4; then
echo "10 0 * * * root /usr/local/bin/reboot_otomatis" > /etc/cron.d/reboot_otomatis
echo -e "${white}Auto-Reboot Berhasil Diaktifkan Setiap 1 Hari.${off}"
elif test $x -eq 5; then
echo "10 0 */7 * * root /usr/local/bin/reboot_otomatis" > /etc/cron.d/reboot_otomatis
echo -e "${white}Auto-Reboot Berhasil Diaktifkan Setiap 1 Minggu.${off}"
elif test $x -eq 6; then
echo "10 0 1 * * root /usr/local/bin/reboot_otomatis" > /etc/cron.d/reboot_otomatis
echo -e "${white}Auto-Reboot Berhasil Diaktifkan Setiap 1 Bulan.${off}"
elif test $x -eq 7; then
rm -f /etc/cron.d/reboot_otomatis
echo -e "${white}Auto-Reboot Berhasil Dinonaktifkan.${off}"
elif test $x -eq 8; then
if [ ! -e /root/log-reboot.txt ]; then
	echo -e "${red}Tidak Ada Aktifitas Reboot${off}"
	else 
	echo -e "${red}LOG REBOOT${off}"
	echo -e"${white}-------"
...
echo -e "${red}Log Reboot Berhasil Dihapus!${off}"
else
